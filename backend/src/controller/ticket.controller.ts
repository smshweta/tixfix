...
const getAllTickets = (req: Request, res: Response) => {
  ...
  ticket.selectAllTickets().then(tickets => { // .then for async call
    if (userType === Role.CUSTOMER) {
      tickets = tickets.filter((ticket) => ticket.customer_id === userid);
    } else if (userType === Role.EMPLOYEE) {
      tickets = tickets.filter((ticket) => ticket.assigned_id === userid);
    }
...
const updateTicketAssignedTo = (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
  const { assigned_uname, userid } = req.body;
  user.selectUserById(userid).then((user) => {
  ...
  user.selectUserByUsername(assigned_uname).then((assigned_user) => {
    if (!assigned_uname || assigned_user.role !== Role.EMPLOYEE) {
      return res.status(400).send({
  ...
  ticket.updateTicketAssignedTo(id, assigned_uname).then((result: Ticket) => {
    res.status(200).send({
      message: 'OK',
      result: result
    })
