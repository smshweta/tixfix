...
const selectAllTickets = (): Promise<Ticket[]> => {
    return new Promise((resolve, reject) => {
        connection.getConnection((err: NodeJS.ErrnoException | null, conn: PoolConnection) => {
            conn.query("select * from ticket", (err: unknown, resultSet: Ticket[]) => {
                conn.release();
                if (err) {
                    return reject(err);
                }
...
const updateTicketAssignedTo = (id: number, assigned_id: number): Promise<Ticket> => {
    return new Promise((resolve, reject) => {
        connection.getConnection((err: NodeJS.ErrnoException | null, conn: PoolConnection) => {
            conn.query(`update ticket set assigned_id = '${assigned_id}' where id = '${id}'`, (err: unknown) => {
                conn.release();
                if (err) {
                    return reject(err);
                }
